...
print("Displaying community page")
...
# graph_reddit = w.graph.largest_connected_component(create_graph_reddit())
# graph_wiki = w.graph.largest_connected_component(create_graph_wiki().to_undirected())
layout_reddit = get_fa2_layout(graph_reddit, saved="reddit_simple_noargs_gcc.json")
...
############ Generate elements #####
####################################
elements, properties = build_cytoscape_elements(
    graph_wiki,
...
stylesheet, legend = make_stylesheet(properties)
...
def display_hover_data(value):
    ...
    if not value or value == "none":
        ...
        stylesheet, legend = make_stylesheet(
            properties, color_node_by="mechanism_category"
        )

    elif value == "mechanism":
        stylesheet, legend = make_stylesheet(
            properties, color_node_by="effect_category"
        )
